...
NSString * const CoreDataManagerErrorDomain = @"CoreDataManagerErrorDomain";
...
- (void) save
{
	...
	if(!success)
    {
        ...
        for (NSError * error in [saveError underlyingErrors]) {
            if ([error.domain isEqualToString:CoreDataManagerErrorDomain] && error.code==NSManagedObjectValidationError) {
